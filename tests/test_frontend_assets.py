...
def test_frontend_registers_static_assets_not_sidebar_panel():
    ...
    assert INTERCOM_CARD_MODULE == "/oldphonekiosk_static/oldphonekiosk-intercom-card.js?v=0.1.46"
...
def test_lovelace_intercom_card_asset_exists_and_is_registered():
    ...
    assert "const hasLocalAudio = await this._ensureLocalAudio()" in content
    assert "new MediaStream([event.track])" in content
    assert "_startInboundAudioStats" in content
    assert "bytesReceived" in content
    ...
    assert "audio.srcObject !== this._remoteStream" in content
    assert "_bindRemoteAudio(false)" in content
    assert "_remoteStream" in content
...
def test_intercom_card_resource_matcher_handles_cache_busted_urls():
    ...
    assert is_intercom_card_resource({"url": "/oldphonekiosk_static/oldphonekiosk-intercom-card.js?v=0.1.46"})
