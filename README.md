...
Home Assistant + this custom integration
        ...
        │
        │ read-only generated URL with display token
        └──────────────▶ Kindle / old browser web display
```
...
| `GET /api/oldphonekiosk/web-display/{device_id}?token=...` | read-only Kindle/old-browser HTML display generated by Home Assistant |
...
### `oldphonekiosk.create_web_display`

Creates a read-only Kindle/old-browser display from Home Assistant. There is no pairing flow on the Kindle: call the service in HA, copy/open the returned `display_url` on the Kindle, then configure the generated display's dashboard/task/calendar sources from the OldPhoneKiosk device controls.
...
Returns `device_id` and `display_url`. The URL contains a revocable display token; do not publish it publicly.
